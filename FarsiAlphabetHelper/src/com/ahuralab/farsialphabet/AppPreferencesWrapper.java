...
public class AppPreferencesWrapper {
	...
	int incrementAppOpenedCounter() {
		...
		boolean result = preferences.edit().putInt(APP_OPENED_COUNTER, count).commit();
		if (!result) {
			// Log that we could not write on prefs file.
			Log.w("AppPreferencesWrapper",
					"Failed to write sahared pref: " + APP_OPENED_COUNTER);
		}
	...
	boolean setShouldAskToRate(boolean value) {
		boolean result = preferences.edit().putBoolean(SHOULD_ASK_TO_RATE, value).commit();
		if (!result) {
			// Log that we could not write on prefs file.
			Log.w("AppPreferencesWrapper",
					"Failed to write sahared pref: " + SHOULD_ASK_TO_RATE);
		}
